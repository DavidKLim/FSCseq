...
Rcpp::List M_step(arma::mat X, arma::vec y_j, int p, int j, int a, int k,
                  ...
                  int cl_phi, int est_covar, int est_beta,
  ...
  for(int i=0; i<maxit_IRLS; i++){
    ...
    while(CDA_conv == 0){
      ...
      if(CDA_index > 0){
        ...
        double diff_b=0;
		//b0.print("b0"); b1.print("b1");
        for(int cc=0; cc<(k+p); cc++){
          diff_b += fabs(b1(cc)-b0(cc))/fabs(b0(cc)*(k+p) + 1e-50);   // theta not part of convergence
        }
		//Rprintf("delta_CDA: %f",diff_b);
        if(diff_b < CDA_tol){
          CDA_conv=1;
        }
        b0=b1;
    ...
    if(i>=1){
      ...
      double diff_b2=0;
	  //b0.print("b"); b1.print("b2");
      for(int cc=0; cc<(k+p); cc++){
        diff_b2 += fabs(b(cc)-b2(cc))/fabs(b2(cc)*(k+p) + 1e-50);   // theta not part of convergence
      }
	  //Rprintf("delta_IRLS: %f",diff_b2);
