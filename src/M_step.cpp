...
//' Perform phi_ml estimation of phi
//' 
//' @param y, mu, wts, limit, p0, trace
//' @export
// [[Rcpp::export]]
...
double phi_ml(arma::vec y, arma::vec mu, arma::vec wts, int limit, double p0, int trace){
    double eps = 0.0001220703; /* from R */
    double N = accu(wts);
    int n = y.size();
    arma::vec wtd_y(n);
...
Rcpp::List M_step(arma::mat X, arma::vec y_j, int p, int j, int a, int k, 
                  ...
                  int cl_phi, int est_phi, int est_covar,
  ...
  if(est_phi==1 && cl_phi==0){
    ...
    return Rcpp::List::create(Rcpp::Named("coefs_j")=b,
                              ...
                              Rcpp::Named("temp_gamma")=temp_gamma,
                              Rcpp::Named("phi_j")=phi_j);
